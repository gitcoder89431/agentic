...
mod tests {
    ...
    async fn test_openrouter_integration() {
        ...
        match result {
            Ok(models) => {
                ...
                let free_models: Vec<_> = models.iter().filter(|m| 
                    m.pricing.prompt == "0" && m.pricing.completion == "0"
                ).collect();
                let paid_models: Vec<_> = models.iter().filter(|m| 
                    m.pricing.prompt != "0" || m.pricing.completion != "0"
                ).collect();
                
                println!("  📊 Found {} free models, {} paid models", free_models.len(), paid_models.len());
                assert!(!free_models.is_empty(), "Should have at least some free models");
                assert!(!paid_models.is_empty(), "Should have at least some paid models");
                
                // Print first few free models for verification
                println!("  🆓 Free models:");
                for (i, model) in free_models.iter().enumerate().take(3) {
                    println!("    {}. {} ({}k context)", i+1, model.name, model.context_length / 1000);
                }
        ...
        match result {
            Ok(models) => {
                ...
                if let Some(first_model) = models.first() {
                    ...
                    let validation_result = validator.validate_local_endpoint(endpoint, &first_model.name).await;
    ...
    async fn test_dual_endpoint_validation() {
        ...
        match (ollama_models, openrouter_models) {
            (Ok(local_models), Ok(cloud_models)) if !local_models.is_empty() && !cloud_models.is_empty() => {
            ...
            (Err(local_err), Ok(cloud_models)) => {
                ...
                println!("✅ Cloud endpoint working with {} models", cloud_models.len());
            },
            (Ok(local_models), Err(cloud_err)) => {
                println!("✅ Local endpoint working with {} models", local_models.len());
            ...
            Ok(models) => {
                ...
                for model in models.iter() {
                    ...
                    if is_free {
                        ...
                        assert!(!found_paid_after_free, "Free models should come before paid models");
                    } else {
                        paid_count += 1;
                ...
                println!("  📊 Model breakdown: {} free, {} paid", free_count, paid_count);
                ...
                for model in models.iter().take(3) {
                    let pricing_type = if model.pricing.prompt == "0" && model.pricing.completion == "0" {
                        "(free)"
                    } else {
                        "(paid)"
                    };
                    println!("  - {} {} ({} tokens)", model.name, pricing_type, model.context_length);
